...
class CastleEngine {
    ...
    /**
     * Déploie tous les 12 châteaux sur la carte aux coordonnées spécifiées
     */
    public function spawnAllCastles(): array {
        ...
        foreach ($castles as $castle) {
            $x = $castle['coord_x'] ?? $castle['default_x'];
            $y = $castle['coord_y'] ?? $castle['default_y'];
            $this->spawnCastle((int)$castle['id'], $x, $y);
            $spawnedCount++;
        }

        return ['success' => true, 'count' => $spawnedCount, 'message' => "Les 12 Châteaux Authentiques du Japon ont été déployés sur la carte !"];
    }
